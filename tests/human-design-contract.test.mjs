...
test("bodygraph result page exposes the Dusk explorer workspace and interpretation sections", () => {
  ...
  assert.match(
    propertiesDrawer,
    /<UnlockReadingCard upgradeHref={upgradeHref}/,
  );
...
test("response page hydrates one React runtime and clears the fixed Dusk spine safely", () => {
  ...
  assert.match(
    styles,
    /\.hd-reading \.hd-bodygraph-page \{[\s\S]*?padding-left:\s*86px/,
  );
  assert.match(
    styles,
    /@media \(max-width: 768px\) \{[\s\S]*?\.hd-reading \.hd-bodygraph-page \{[\s\S]*?padding-left:\s*0/,
  );
});
